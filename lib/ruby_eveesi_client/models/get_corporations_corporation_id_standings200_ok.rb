  ...
  class GetCorporationsCorporationIdStandings200Ok
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RubyEveesiClient::GetCorporationsCorporationIdStandings200Ok` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RubyEveesiClient::GetCorporationsCorporationIdStandings200Ok`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] from_type Object to be assigned
    def from_type=(from_type)
      validator = EnumAttributeValidator.new('String', ["agent", "npc_corp", "faction"])
      unless validator.valid?(from_type)
