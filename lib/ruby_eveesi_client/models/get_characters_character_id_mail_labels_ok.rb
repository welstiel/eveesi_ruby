  ...
  class GetCharactersCharacterIdMailLabelsOk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RubyEveesiClient::GetCharactersCharacterIdMailLabelsOk` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RubyEveesiClient::GetCharactersCharacterIdMailLabelsOk`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'total_unread_count')
    ...
    def list_invalid_properties
      ...
      if !@labels.nil? && @labels.length > 30
        invalid_properties.push('invalid value for "labels", number of items must be less than or equal to 30.')
      end
    ...
    def valid?
      return false if !@labels.nil? && @labels.length > 30
    ...
    def labels=(labels)
      if !labels.nil? && labels.length > 30
        fail ArgumentError, 'invalid value for "labels", number of items must be less than or equal to 30.'
      end
