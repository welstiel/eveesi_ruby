  ...
  class GetUniverseConstellationsConstellationIdOk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RubyEveesiClient::GetUniverseConstellationsConstellationIdOk` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RubyEveesiClient::GetUniverseConstellationsConstellationIdOk`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @systems.length > 10000
        invalid_properties.push('invalid value for "systems", number of items must be less than or equal to 10000.')
      end
    ...
    def valid?
      ...
      return false if @systems.length > 10000
      true
    end
    ...
    def systems=(systems)
      if systems.nil?
        fail ArgumentError, 'systems cannot be nil'
      end

      if systems.length > 10000
        fail ArgumentError, 'invalid value for "systems", number of items must be less than or equal to 10000.'
      end
    ...
    def hash
      [constellation_id, name, position, region_id, systems].hash
    end
