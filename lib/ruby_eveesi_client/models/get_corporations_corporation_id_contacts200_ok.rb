  ...
  class GetCorporationsCorporationIdContacts200Ok
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RubyEveesiClient::GetCorporationsCorporationIdContacts200Ok` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RubyEveesiClient::GetCorporationsCorporationIdContacts200Ok`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@label_ids.nil? && @label_ids.length > 63
        invalid_properties.push('invalid value for "label_ids", number of items must be less than or equal to 63.')
      end
    ...
    def valid?
      ...
      return false if !@label_ids.nil? && @label_ids.length > 63
    ...
    def label_ids=(label_ids)
      if !label_ids.nil? && label_ids.length > 63
        fail ArgumentError, 'invalid value for "label_ids", number of items must be less than or equal to 63.'
      end
